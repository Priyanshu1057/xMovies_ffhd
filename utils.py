...
from info import LONG_IMDB_DESCRIPTION, IS_VERIFY, START_IMG, LOG_CHANNEL, DELETE_TIME, CAPTCHA_SITE
...
# 🚀 Modified shortener with captcha wrapper
async def get_shortlink(link, grp_id, is_second_shortener=False, is_third_shortener=False):
    ...
    if IS_VERIFY:
        ...
        # Wrap with captcha site if configured
        if CAPTCHA_SITE:
            link = f"{CAPTCHA_SITE}/verify.php?redirect={link}"
